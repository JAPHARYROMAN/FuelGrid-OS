...
export default function SetupPage() {
  ...
  const updateStep = useMutation({
    mutationFn: (stepCode: string) =>
      api.updateSetupStep({ step_code: stepCode, status: 'completed' }),
    onSuccess: (data) => {
      qc.setQueryData(['setup-checklist'], data);
    },
  });
  ...
  return (
    <div className="flex flex-col gap-7">
      <PageHeader
        ...
        title="Setup"
        description="Track tenant readiness from the persisted setup checklist and the live master-data contracts."
        actions={
          checklist.isPending ? (
