...
type tankDTO struct {
	...
	// CurrentLitres is the latest dip-resolved volume, an exact decimal STRING
	// (numeric(14,3) text); never Go float64. Populated only by the station
	// overview (from tank_dip_readings); nil elsewhere. The companion fields
	// describe how current that reading is, so the dashboard can flag a stale
	// (prior-day) level rather than present it as today's.
	CurrentLitres          *string `json:"current_litres,omitempty"`
	CurrentDipReadingType  *string `json:"current_dip_reading_type,omitempty"`
	CurrentDipRecordedAt   *string `json:"current_dip_recorded_at,omitempty"`
	CurrentDipBusinessDate *string `json:"current_dip_business_date,omitempty"`
}
