...
The "with conditions" qualifier is driven by **two confirmed Medium findings** that are real and exploitable via direct API calls but are not data-exfiltration or tenant-isolation breaks:

1. **MFA requirement is not enforced server-side** — privileged roles (admin/finance) that never enrolled a second factor are not blocked by the API; enforcement is UI-side only.
2. **Payment recording has no idempotency guard** — a retry or replay can double-record a shift tender, inflating cash/revenue counts.

Neither is deploy-blocking for a controlled launch, but both should be on the pre-launch checklist (Section 4). No genuinely Critical, deploy-blocking issue was confirmed.
...
- [ ] **SR-M1 — Enforce MFA server-side for privileged roles.** Add `requireMFASatisfied` middleware on user/role-management, approval, and governance routes; reject privileged-but-unenrolled sessions with 403. Until shipped, ensure all privileged accounts (system_admin/tenant_admin/finance) enroll MFA via the UI *before* go-live, and restrict direct API access. Fix the misleading comment at `internal/identity/service.go:1000-1002`.
- [ ] **SR-M2 — Add payment idempotency.** Introduce an idempotency key + partial unique constraint on `(tenant_id, shift_id, idempotency_key)` and return the existing record on conflict; or formally document reconciliation as the dedup authority and monitor shift variance.
