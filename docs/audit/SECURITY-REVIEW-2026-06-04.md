...
|----|---------|----------------------|--------|-------------|----------------|
| SR-L1 | **No shift-closure check before recording a payment.** `handleRecordPayment` loads the shift and authorizes the station but never checks shift status before `payments.Record()`. If shift closure is meant to be terminal, payments can still post to a closed shift. | `services/api/internal/server/payments_handlers.go:87-98` | A tender can be recorded against an already-closed shift. Whether this is a gap or intentional (post-close corrections) is a product decision. | Confirm intent; if closure is terminal, reject with 409 when `shift.Status == "closed"`. Otherwise document that post-close recording is allowed for corrections. | Payments — S. **CLOSED — by-design** (product decision 2026-06-05): post-close recording is intentionally allowed to support late corrections/adjustments; closure is not terminal for tender recording. Station-scope authorization + audit logging remain the controls. No code change. |
| SR-L2 | **RESOLVED** (branch `fix/sec-low-info-hardening`): the logo download now sets `X-Content-Type-Options: nosniff` and `Content-Disposition: inline`, matching the attachments handler's defense-in-depth (`services/api/internal/server/branding_handlers.go:~287`). Test: `TestLetterhead_LogoUploadValidation` asserts both headers. Original finding below. — **Logo download missing `X-Content-Type-Options: nosniff`.** The download re-serves the stored content-type without `nosniff`, unlike the attachments handler which sets both `nosniff` and a content disposition. Upload strictly validates PNG/JPEG via `http.DetectContentType`, so escalation to script execution is not plausible. | `services/api/internal/server/branding_handlers.go:287-290` | Defense-in-depth inconsistency; very low exploitability (content-type validated at upload, image types only). | Add `X-Content-Type-Options: nosniff` (and an `inline` content disposition) to the logo download for parity with `attachments_handlers.go`. | Platform/Assets — S |
| SR-L3 | **RESOLVED** (branch `fix/sec-low-info-hardening`): added a per-IP HTTP-layer limiter on the public `/auth/password-reset/request` and `/confirm` routes, reusing the Redis-backed `ratelimit.Limiter` under a dedicated `ratelimit:pwreset:` prefix (keyed by `clientIP`, fail-open, lenient default 10 / 15 min via `AUTH_PASSWORD_RESET_RATE_LIMIT`/`_WINDOW`). `services/api/internal/server/ratelimit_middleware.go` (`rateLimitPasswordResetIP`), wired in `server_routes.go`. Test: `TestSRL3_PasswordResetConfirmRateLimited` proves repeated same-IP confirms hit 429. Original finding below. — **`/auth/password-reset/confirm` has no HTTP-layer rate limit.** The route is registered outside any rate-limit group; the per-tenant limiter applies only to authenticated routes. (`/login` itself is protected by the identity service's dual IP+account buckets.) | `services/api/internal/server/server_routes.go:154-155`; `services/api/internal/server/ratelimit_middleware.go` | Low: each confirm requires a unique one-time token hashed in Redis (1h TTL, cleared on use), so there is no high-yield brute-force surface. Risk is limited to an attacker who already intercepted a reset token. | Apply a per-IP/per-token limit (e.g. 3 attempts / 15 min) on confirm, or rate-limit at the reverse proxy. | Identity/Auth — S |
| SR-L4 | **RESOLVED** (branch `fix/sec-low-info-hardening`): `GrantScope` now performs a tenant-scoped existence check for any non-`tenant` scope — `station`/`company`/`region`/`group` `scope_id` must resolve to a row owned by the caller's tenant or the grant is rejected with 400 (`enterprise.ErrScopeTargetNotFound`). `internal/enterprise/governance.go` (`GrantScope`) + `services/api/internal/server/enterprise_governance_handlers.go`. Test: `TestSRL4_GrantScopeRejectsCrossTenantScopeID` proves cross-tenant/unknown ids are rejected and same-tenant ids succeed. Original finding below. — **Enterprise scope grants lack `scope_id` tenant-ownership validation.** `GrantScope` does not verify that a non-`tenant` `scope_id` belongs to the granting tenant. | `services/api/internal/server/enterprise_governance_handlers.go:119-140`; `internal/enterprise/governance.go:97-104`; `services/api/migrations/0057_enterprise_governance.up.sql:45-61` | Low: enterprise scopes drive UI filtering/navigation only — authorization is enforced by the separate `policy.LoadFor()` + `authorizeStation()` path, so a cross-tenant scope grant grants no actual resource access. Defensive hardening only. | Add an application-level (or CHECK/FK) validation that `scope_id` resolves to a row owned by the tenant for `station`/`company` scope types. | Enterprise — S |
...
| SR-I2 | **RESOLVED** (branch `fix/sec-low-info-hardening`): added conservative `BeforeSend` + `BeforeBreadcrumb` scrubbers in `internal/observability/sentry.go` that redact emails, bearer/authorization tokens, password/secret/token assignments and phone numbers from event messages, exception values, request data/query/headers/cookies, tags and breadcrumbs before anything leaves the process. Test: `TestScrubString`/`TestScrubEvent`/`TestScrubBreadcrumb`. Original note below. — Sentry has no active breadcrumb scrubber. No PII reaches Sentry today (only request_id/method/status/route tags), but future breadcrumbs added in auth paths could leak. | `internal/observability/sentry.go:36` | Proactively install a `BeforeSend`/breadcrumb filter that redacts email/phone/token patterns before any auth-path breadcrumbs are introduced. |
...
- [x] SR-L1 — Decide and enforce/document post-close payment behavior. **CLOSED — by-design** (product decision 2026-06-05): post-close payment recording is intentionally allowed for corrections; no code change.
- [x] SR-L2 — Add `nosniff` to logo download. **RESOLVED** (branch `fix/sec-low-info-hardening`): logo download now sends `X-Content-Type-Options: nosniff` + `Content-Disposition: inline`.
- [x] SR-L3 — Rate-limit `/auth/password-reset/confirm` (per-IP/token). **RESOLVED** (branch `fix/sec-low-info-hardening`): per-IP limiter on the request/confirm routes reusing the Redis `ratelimit.Limiter` (lenient default 10 / 15 min, fail-open).
- [x] SR-L4 — Validate enterprise `scope_id` tenant ownership. **RESOLVED** (branch `fix/sec-low-info-hardening`): `GrantScope` tenant-scoped existence check rejects cross-tenant/unknown non-`tenant` `scope_id` with 400.
- [x] SR-I2 — Install a Sentry breadcrumb/PII scrubber before adding auth-path breadcrumbs. **RESOLVED** (branch `fix/sec-low-info-hardening`): `BeforeSend`/`BeforeBreadcrumb` redact email/token/password/phone from outbound events.
