...
- [x] Deployment target: **DigitalOcean Droplet** (`docker compose` + self-hosted Postgres/Redis behind Caddy), documented in [docs/deployment.md](docs/deployment.md) with topology, the production compose stack in [`deploy/`](../deploy/), secret inventory, migration discipline, and the branch-protection command.
